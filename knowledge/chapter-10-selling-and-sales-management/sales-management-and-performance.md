---
type: Sales Management Framework
title: Sales management and performance
description: How direction, feedback, support, and activity-based control improve sales performance.
status: draft
tags:
  - chapter-10
  - sales-management
  - performance
generated:
  by: codex/gpt-5
sources:
  - id: original-pdf
    title: "Marketing: Theory, Evidence, Practice, second edition"
    resource: /knowledge/release-information/source-provenance.md
---
# Sales management and performance

## Core statement

Sales management sets expectations, supports selling activities, evaluates performance, and provides feedback, coaching, and training. Activity-based evaluation helps diagnose where intervention is needed rather than relying only on results.[^management][^performance]

## Decision implications

Define role clarity, performance criteria, support, feedback cycles, and training needs for each selling stage. Align sales strategy and rewards with wider business and marketing strategy.[^management]

[^management]: [Primary PDF source](/knowledge/release-information/source-provenance.md), Chapter 10, printed pages 412–415; PDF pages 450–453.
[^performance]: [Primary PDF source](/knowledge/release-information/source-provenance.md), Chapter 10, printed pages 416–419; PDF pages 454–457.